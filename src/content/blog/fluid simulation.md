...
The second type is the *Neumann Boundary Condition*, where instead of fixing the value of the quantity itself, we fix its derivative — specifically the derivative in the direction normal to the boundary. In our case we set $\frac{\partial p}{\partial n} = 0$, which means the pressure does not change as you move perpendicularly into the boundary. Intuitively this means there is no pressure gradient pushing fluid through the wall, which is consistent with the no-flow condition we already imposed on the velocity. In practice, when solving the Poisson equation near a solid boundary, this is enforced by treating the pressure of a solid neighbor as equal to the pressure of the fluid cell next to it — so the gradient across that face is zero and no spurious pressure force is introduced.
